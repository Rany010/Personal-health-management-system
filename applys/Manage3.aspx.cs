...
public partial class applys_List : System.Web.UI.Page
{
    ...
    private void bind()
    {
        string where = " where dno='" + Session["bh"].ToString() + "' ";


        GridView1.DataSource = DbHelperSQL.Query("select * from applys a left join users b on a.lname=b.lname " + where + " order by id desc");
        GridView1.DataBind();
